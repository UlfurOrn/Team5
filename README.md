...
In sprint 3 we kind of got a conditional pass for our dependency injection not being quite correct. So we decided to rather talk about our other dependency injection which can be found in the mail service. The mail_service.py file contains a class that takes in another class which can then be used. This class allows for a quick and easy change of classes by simply changing the inserted class. We hope that this class follows the correct dependency injection pattern.
